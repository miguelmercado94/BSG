#!/bin/sh
set -e
# Puerto de escucha (Railway asigna PORT dinámico; Compose por defecto 80).
export PORT="${PORT:-80}"
...
if [ -f "$NGINX_TMPL" ]; then
  envsubst '$PORT $DOCVIZ_PROXY_HOST $DOCVIZ_PROXY_PORT $SECURITY_PROXY_HOST $SECURITY_PROXY_PORT $NGINX_RESOLVER' <"$NGINX_TMPL" >/etc/nginx/conf.d/default.conf
fi
