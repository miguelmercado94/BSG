CREATE OR REPLACE TRIGGER SIM_TRG_MAXAGEE_908_C2200050
AFTER UPDATE OR INSERT  ON C2200050
FOR EACH ROW

--Solo se hace el cambio cuando se modifica la cobertura ITP para productos vida individual
WHEN ((
          NEW.Cod_Ramo IN(940,942,944,946)
      AND NEW.cod_cob  IN(908)
     ))

/*
Created by: Juan González
Date:07 11 2018
Porpouse: Guardar la edad máxima tabla parametro MAX_AGE_908
