CREATE OR REPLACE TRIGGER TRG_VPA_CONVENIO_FACT_E
AFTER INSERT OR UPDATE OF ESTADO_REGISTRO, ESTADO_CTB ON SB_CONVENIO
...
WHEN ((new.estado_registro = 'CTB' OR new.estado_ctb = 'CTB') AND NEW.VALOR_FPU != 0)
  ...
  ----------------------------------------------------------------------------------------
  /******************************************
  -- TRG_VPA_CONVENIO_FACT_E: Trigger encargado de registrar una factura electronica cada vez
                              que se contabiliza una factura de pago en SB_CONVENIO con valor de ahorro(FPU)
  --
...
DECLARE
  ...
  Vl_Consec_Nota   SB_CONVENIO.CONSECUTIVO%TYPE := NULL;
  Vl_Consec_Desc   SB_CONVENIO.DESCRIPCION_NO_PAGO%TYPE := NULL;
  --
  Vl_Org           CONSTANT VARCHAR2(50) := 'TRG_VPA_CONVENIO_FACT_E';
  Vl_Msj           VARCHAR2(1000);
  Vl_Tip           VARCHAR2(100);
  Vl_Sec           SB_CONVENIO.CONSECUTIVO%TYPE := :new.consecutivo;
  Vl_Pol           SB_CONVENIO.NUMERO_POLIZA%TYPE := :new.numero_poliza;
...
BEGIN
  ...
  BEGIN
    ...
    IF :new.valor_fpu < 0 THEN
      BEGIN
        SELECT consecutivo, descripcion_no_pago
          INTO Vl_Consec_Nota, Vl_Consec_Desc
          FROM sb_convenio
         WHERE seccion       = :new.seccion
           AND producto      = :new.producto
           AND numero_poliza = :new.numero_poliza
           AND descripcion_no_pago LIKE '%'||:new.consecutivo||'%';
         
        Vg_Reg.Consec_Nota      := Vl_Consec_Nota;
        Vg_Reg.Descripcion_Nota := Vl_Consec_Desc;
      EXCEPTION
        WHEN OTHERS THEN
