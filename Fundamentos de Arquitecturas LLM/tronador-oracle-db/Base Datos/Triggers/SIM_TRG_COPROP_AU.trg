CREATE OR REPLACE TRIGGER SIM_TRG_COPROP_AU
AFTER UPDATE --OF NRO_DOCUMTO
...
BEGIN  
  ...
  IF((:NEW.COD_CIA = 3) AND (:NEW.COD_SECC = 4) AND (:NEW.COD_RAMO = 445) AND (:OLD.NRO_DOCUMTO <> :NEW.NRO_DOCUMTO)) THEN
  ...
  */
  ...
  EXCEPTION
  WHEN OTHERS THEN
     sim_proc_log('SIM_TRG_COPROP_AU',sqlcode||' - '||sqlerrm);
