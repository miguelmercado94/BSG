CREATE OR REPLACE TRIGGER TRG_BIU_R_A3001700_TER
...
IF INSERTING THEN
  IF :NEW.TDOC_TERCERO IS NULL OR :NEW.SEC_TERCERO IS NULL THEN
    BEGIN
      INSERTA_C1991300(:NEW.COD_BENEF,'A3001700','I','10','El programa que genera los datos no esta procesando correctamente la informacion del tercero. ',
    ...
    END;
  --  :NEW.SEC_TERCERO := NULL; no se debe asignar nulo a la secuencia
  If :New.Sec_Tercero Is Null Then
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ...
        END;
     END;
  End If; --Si la secuencia no es nula no se hace nada
  END IF;
ELSE
  DECLARE
...
EXCEPTION
 WHEN OTHERS THEN
  BEGIN
     v_coderr := sqlcode;
     v_msgerr := sqlerrm;
     INSERTA_C1991300(:NEW.COD_BENEF,'A7000900','T',v_Coderr,V_MsgErr,
        user,SYSDATE,:NEW.TDOC_TERCERO,:NEW.SEC_TERCERO);
     EXCEPTION
        WHEN OTHERS THEN null;
        -- RAISE_APPLICATION_ERROR(-20501,'Error insertando el control que los campos estaban nulos.');
