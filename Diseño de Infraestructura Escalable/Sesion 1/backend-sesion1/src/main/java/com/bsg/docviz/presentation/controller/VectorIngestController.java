...
import com.bsg.docviz.dto.WorkAreaIngestRequest;
...
public class VectorIngestController {
    ...
    /**
     * Indexa un archivo del área de trabajo (borrador aceptado por el usuario) en el mismo namespace que el repo.
     */
    @PostMapping("/vector/work-area/ingest")
    public ResponseEntity<VectorIngestResponse> ingestWorkArea(@Valid @RequestBody WorkAreaIngestRequest body) {
        return ResponseEntity.ok(vectorIngestService.ingestWorkAreaFile(body.getFileName(), body.getContent()));
    }
